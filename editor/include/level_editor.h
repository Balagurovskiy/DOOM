...
t_ibutton			create_inc_button(int max_value, int min_value,
						t_texture minus, t_texture plus);
int					handle_btn_click(t_button button, int x, int y);
int					handle_inc_btn_click(t_ibutton button,
...
int					is_line_intersect_shapes(t_app *app,
						t_point p1, t_point p2);
t_line				get_line(int x0, int y0, int x1, int y1);
void				update_shape_neighbors(t_app *app,
						t_line line, int shape_id);
int					check_point_neighbor_match(t_shape *shape,
						int index, t_point p);
...
void				create_text(t_app *app,
						char *text, t_text *text_obj, t_point point);
